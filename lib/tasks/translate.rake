...
namespace :i18n do
  ...
  task :untranslated => :environment do
    ...
    untranslated.each do |locale, keys|
      keys.each do |key|
        ...
        a_key = Translate::Keys.to_deep_hash({key => from_text})
        all_missing = all_missing.deep_merge(a_key)
      end
    end

    if messages.present?
      messages.each { |m| puts m }
      # http://iain.nl/writing-yaml-files
      all_missing.deep_stringify_keys!
      #binding.pry
      File.open("tmp.yml", 'w') {|f| f.write(all_missing.to_yaml) }
      #write("tmp.yml", all_missing)
    else
  ...
  desc "Remove all translation texts that are no longer present in the locale they were translated from"
  task :remove_unnused_keys => :environment do
    I18n.backend.send(:init_translations)
    master_locale = ENV['LOCALE'] || I18n.default_locale
    texts = {}
    files = Dir["app/**/*.rb","app/**/*.haml","app/**/*.erb"].reject{ |fn| File.directory?(fn) }.map{ |i| IO.readlines(i) }
    keys = Translate::Keys.to_shallow_hash(Translate::File.new("config/locales/application.en.yml").read)
    binding.pry
    files.each do |file|
      keys.each do |key|
        search_key = key[0].to_s.gsub('en.','')
        if file.grep(/#{search_key}/).length > 0
          texts["en." + search_key] = key[1].to_s
        else
          #p "could not find: " + search_key
        end
      end
      # I18n.backend.send(:translations)[locale] = nil # Clear out all current translations
      # I18n.backend.store_translations(locale, Translate::Keys.to_deep_hash(texts))
